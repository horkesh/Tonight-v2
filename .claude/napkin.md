...
5. **[2026-05-16] Vercel CLI is not installed — use the `claude.ai_Vercel` MCP tools instead**
   Do instead: to verify a deploy or inspect logs, use the deferred MCP tools (`mcp__claude_ai_Vercel__list_deployments`, `get_deployment`, `get_deployment_build_logs`). Load them via `ToolSearch` first. The team is `team_SpPoZYOLWh3JTJjuvfZG10Xn` ("Haris' projects"); this app's project is `tonight-v2` (`prj_DnZEodDJGC59V2ZaCyIqiuA5gYRz`). Don't waste time trying `vercel deploy` / `vercel inspect` / `vercel logs` from the shell — they'll fail with "command not found".
